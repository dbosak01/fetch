...
### Apply a Filter to Fetch Operation
...
### Apply a Filter to an Entire Catalog

Sometimes you want to subset all the datasets in your catalog.  This subset
can be accomplished with the "filter" parameter on the `catalog()` function.
Observe:
...
Notice that a filter has been added to each data item in the catalog, 
...
Date columns.  The format "%d%b%Y" was written specifically to read in the 
